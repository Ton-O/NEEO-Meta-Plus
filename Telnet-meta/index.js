...
function metaLog(message) {
  let initMessage = { component:'telnet', type:LOG_TYPE.INFO, content:'', deviceId: null };
...
class Telnet extends events_1.EventEmitter {
        ...
        try {
            ...
            if (this.opts.extSock) {
                ...
                this.socket = (0, net_1.createConnection)(Object.assign({ port: this.opts.port, host: this.opts.host, localAddress: this.opts.localAddress }, this.opts.socketConnectOptions), () => {
                    this.state = 'start';
                    this.emit('connect');
                    metaLog({type:LOG_TYPE.VERBOSE, content:"Connection was made"});
            ...
            this.socket.setTimeout(this.opts.timeout, () => {
                if (connectionPending) {
                    metaLog({type:LOG_TYPE.VERBOSE, content:"Socket timeout With pending connection request"});
                    // If cannot connect, emit error and destroy.
                    if (this.listeners('error').length > 0)
                        this.emit('error', 'Cannot connect');
                    this.socket.destroy();
                    this.emit('timeout');
                    return reject(new Error('Timeout in connecting'));
                }
                return resolve('') // ject(new Error('timeout'));
            });
            ...
            this.socket.on('error', error => {
                metaLog({type:LOG_TYPE.DEBUG, content:"error-event on socket"});
                if (this.listeners('error').length > 0)
            ...
            this.socket.on('end', () => {
                metaLog({type:LOG_TYPE.DEBUG, content:"end-event on socket"});
                if (!this.endEmitted) {
            ...
            this.socket.on('close', () => {
                metaLog({type:LOG_TYPE.DEBUG, content:"close-event on socket"});
                this.emit('close');
                if (connectionPending) {
                    if (this.state === 'start')
                        resolveIt();
            ...
            this.once('failedlogin', () => {
                metaLog({type:LOG_TYPE.DEBUG, content:"failedlogin-event on socket"});
                if (connectionPending)
                    rejectIt('Failed login');
            });
        }
    ...
    exec(cmd, opts, callback) {
        metaLog({type:LOG_TYPE.VERBOSE, content:"Call to exec "+cmd});
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return (0, utils_1.asCallback)(new Promise((resolve, reject) => {
                ...
                if (!this.socket.writable)
                    return reject('socket not writable');
                metaLog({type:LOG_TYPE.DEBUG, content:"Call to socket write "+cmd});
                this.state = 'response';
                ...
                this.socket.write(cmd, () => {
                    ...
                    const buffExecHandler = () => {
                        ...
                        if (!this.inputBuffer)
                            return reject('response not received');
                    ...
                    const responseHandler = () => {
                        if (execTimeout)
                            clearTimeout(execTimeout);
                        if (this.response)
                            {metaLog({type:LOG_TYPE.DEBUG, content:"Resolving exec "+cmd+ " with response:"+this.response})
    ...
    send(data, opts, callback) {
        var _a;
        metaLog({type:LOG_TYPE.VERBOSE, content:"Call to send "+data});
        return __awaiter(this, void 0, void 0, function* () {
            if (typeof opts === 'function') {
                callback = opts;
                opts = undefined;
            ...
            data += this.opts.ors;
            metaLog({type:LOG_TYPE.VERBOSE, content:"Calling telnet write"});
            return this.write(data, opts, callback);
        });
    }
    write(data, opts, callback) {
        metaLog({type:LOG_TYPE.VERBOSE, content:"Call to write "+data});
        return __awaiter(this, void 0, void 0, function* () {
            if (typeof opts === 'function') {
                callback = opts;
                opts = undefined;
    ...
    parseData(chunk, isReady) {
        metaLog({type:LOG_TYPE.DEBUG, content:'parseData '+chunk});
        ...
        if (this.state === 'getprompt') {
            ...
            metaLog({type:LOG_TYPE.DEBUG, content:'parseData, handling response'});
            const stringData = this.decode(chunk);
            ...
            chunk = null;
            metaLog({type:LOG_TYPE.VERBOSE, content:"Response ready: "+this.response})
